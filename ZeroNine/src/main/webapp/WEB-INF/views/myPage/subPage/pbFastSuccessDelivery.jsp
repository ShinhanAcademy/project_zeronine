				...
				<c:choose>
					...
					<c:otherwise>
						<button class="btnFirst" id="btnFirst"
							onclick="javascript:callSuccessFastDelivery(1)">첫페이지</button>
					</c:otherwise>
				</c:choose>
				<c:choose>
					<c:when test="${boardSuccessPaging.pageNumber<=1}">
					...
					<c:otherwise>
						<button class="btnPrev" id="btnPrev"
							onclick="javascript:callSuccessFastDelivery(${boardSuccessPaging.pageNumber-1})">이전페이지</button>
					</c:otherwise>
				</c:choose>
			</div>
			<ul class="pagingWrap">
				<c:forEach begin="${boardSuccessPaging.startPageNumber}"
					...
					<li><a href="javascript:callSuccessFastDelivery(${i})">${i}</a></li>
				</c:forEach>
			</ul>
			<div class="pageRight">
				<c:choose>
					...
					<c:otherwise>
						<button class="btnNext" id="btnNext"
							onclick="javascript:callSuccessFastDelivery(${boardSuccessPaging.pageNumber+1})">다음페이지</button>
					</c:otherwise>
				</c:choose>
				<c:choose>
					<c:when test="${boardSuccessPaging.pageNumber==boardSuccessPaging.maxPageNumber}">
					...
					<c:otherwise>
						<button class="btnLast" id="btnLast"
							onclick="javascript:callSuccessFastDelivery(${boardSuccessPaging.maxPageNumber})">마지막페이지</button>
					</c:otherwise>
				</c:choose>
			</div>
		</div>
